    ...
    const getTextLabel = (attribute, task) => {
        ...
        return (
            <div style={{backgroundColor: 'transparent'}} key={attribute._id}>
                <input type="text" name={attribute._id} value={taskOption.value} readOnly />
            </div>
        )
    }

    if (board === null) return <div className={classes.loading}><CircularProgress /></div>
                     ...
                     <div className="task" key={taskGroup._id}>
                         <div className="title">
                             <div>
                                 ...
                                 <p className="task-amount">{taskGroup.tasks.length} TASKS</p>
                             </div>
                                 ...
                                 <div className="Task" key={task._id}>
                                     ...
                                     <div>
                                         {board.attributes.map(attribute => {
                                             if (attribute.type === "status") return getStatusLabel(attribute, task)
                                             if (attribute.type === "text") return getTextLabel(attribute, task)
